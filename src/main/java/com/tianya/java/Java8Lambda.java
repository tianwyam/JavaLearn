...
public class Java8Lambda {
	...
	public void filter() {
		...
		// 获取所有学生的年龄和
		Integer ageSum = students.stream().reduce(0, (a,b)-> a + b.getAge(), (a,b)-> a + b);
		System.out.println("所有学生的年龄总和：" + ageSum);
		
		
		// 转换成MAP
		Map<Integer, String> mapList = students.stream().collect(Collectors.toMap(Student::getSid, Student::getName));
		mapList.forEach((k,v)->System.out.println(k + ": " + v));
		
		
		// 根据 地方分组，求出 每个 地方最高的 身高
		Map<String, Optional<Integer>> groupByList = students.stream().collect(Collectors.groupingBy(Student::getAddr, 
		...
		Map<String, Optional<Student>> groupMaxHeightStudentMap = students.stream().collect(Collectors.groupingBy(Student::getAddr, 
	...
	public static List<Student> initDatas() {
		...
		students.add(Student.builder().sid(15).name("高渐离").age(26).addr("陕西").height(194).build());
		...
		students.add(Student.builder().sid(19).name("王阳明").age(50).addr("四川").height(174).build());
